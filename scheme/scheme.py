...
def apply_primitive(procedure, args, env):
    ...
    try:
        return procedure.fn(*python_lst)
    except TypeError:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            ...
            for n in range(len(vals)):
                frame.bindings[formals[n]] = vals[n]
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(vals[0].first):
            ...
            env.define(vals[0].first, scheme_eval(lambda_exp, env))
            return vals[0].first
        else:
            raise SchemeError
    else:
...
def do_let_form(vals, env):
    ...
    if scheme_listp(bindings):
        for n in bindings:
            if len(n)> 2:
                raise SchemeError
            if not scheme_symbolp(n.first):
                raise SchemeError
            
            values = Pair(scheme_eval(n[1],env), values)
            names = Pair(n.first, names)
...
def do_or_form(vals, env):
    ...
    if len(vals) == 0:
        return False
    Ryan = scheme_eval(vals.first,env)
    if scheme_true(Ryan):
