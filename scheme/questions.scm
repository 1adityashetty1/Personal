...
(define (zip pairs)
  'YOUR-CODE-HERE
  (cons (apply-to-all car pairs) (list (apply-to-all cadr pairs))))
...
(define (list-partitions total max-pieces max-value)
  'YOUR-CODE-HERE
  (define empty '())
  (define (partitioner total max-pieces max-value parts)
    (cond ((= total 0) (list parts))
      ((or (= max-pieces 0) (< total 0)  (< max-value 1)) empty) 
      (else (append
      (partitioner (- total max-value) (- max-pieces 1) max-value (append parts (list max-value)))
      (partitioner total max-pieces (- max-value 1) parts)))))
    (partitioner total max-pieces max-value empty)
)
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               ...
               (body   (cddr expr)))
           'YOUR-CODE-HERE
           (cons form (cons params (append (apply-to-all analyze body) nil)))
           ))
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (append (cons (cons 'lambda (cons (car (zip values)) (apply-to-all analyze body)
           )) nil) (apply-to-all analyze (cadr (zip values)) ))) )
