...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            assert self.ant is None or self.ant.can_contain(insect) or\
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if type(insect) is QueenAnt and insect.index is 1:
                return
            elif type(insect) is BodyguardAnt and insect.ant is not 0:
                self.ant = insect.ant
            else:
                self.ant = None
...
class Ant(Insect):
    ...
    def can_contain(self,other):
        if self.container is True and self.ant is None and other.container is False:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        position = self.place
        distance = 0
        while position.entrance != None or position != hive:
            if len(position.bees)> 0 and distance >= self.min_range and distance <= self.max_range:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in self.place.bees[:]:
                bee.reduce_armor(self.damage)   #iterate throughout list of bees
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        BuffedAnts = []
        self.BuffedAnts = BuffedAnts
        QueenAnt.number_of_queens += 1
        self.index = self.number_of_queens  # distinguish impostors in remove insect
        Ant.__init__(self)
    ...
    def action(self, colony):
        ...
        if self.index > 1:
            ...
            path_exit = self.place.exit
            path_entrance= self.place.entrance

            if self.place.ant.name == 'Bodyguard' and self.place.ant not in self.BuffedAnts: #the queen in bodyguard
                self.place.ant.damage = 2 * self.place.ant.damage
                self.BuffedAnts = self.BuffedAnts + [self.place.ant]
            while path_entrance != None:
                if path_entrance.ant != None:
                    if path_entrance.ant.name == 'Bodyguard' and path_entrance.ant.ant not in self.BuffedAnts:
                        path_entrance.ant.ant.damage = 2 * path_entrance.ant.ant.damage
                        self.BuffedAnts = self.BuffedAnts + [path_entrance.ant.ant]
                    if path_entrance.ant not in self.BuffedAnts:
                        path_entrance.ant.damage = 2 * path_entrance.ant.damage
                        self.BuffedAnts = self.BuffedAnts + [path_entrance.ant]
                path_entrance = path_entrance.entrance
            while path_exit != None:
                if path_exit.ant != None:
                    if path_exit.ant.name == 'Bodyguard' and path_exit.ant.ant not in self.BuffedAnts:
                        path_exit.ant.ant.damage = 2 * path_exit.ant.ant.damage
                        self.BuffedAnts = self.BuffedAnts + [path_exit.ant.ant]
                    if path_exit.ant not in self.BuffedAnts:
                        path_exit.ant.damage = 2 * path_exit.ant.damage
                        self.BuffedAnts = self.BuffedAnts + [path_exit.ant]
                path_exit = path_exit.exit
...
def make_slow(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def make_slow_action(colony):
         if colony.time % 2 == 0:
            action(colony)
    return make_slow_action
...
def apply_effect(effect, bee, duration):
    ...
    def action(colony):
        nonlocal duration
        if duration <= 0:
            return default (colony)
        else: duration -= 1
        return effect(default)(colony)
    bee.action = action
